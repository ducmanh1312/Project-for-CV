...
//#define MY_GATEWAY_SERIAL  // chinh sua
...
//#define SIGN_DEBUG(x,...) debug(x, ##__VA_ARGS__)
  #define SIGN_DEBUG(x,...) printf(x, ##__VA_ARGS__) // chỉnh sửa
#else
  #define SIGN_DEBUG(x,...)
#endif
...
void signerPresentation(MyMessage &msg, uint8_t destination)
{
	...
	if (destination == GATEWAY_ADDRESS) {
		...
		wait_process(2000, C_INTERNAL, I_SIGNING_PRESENTATION);
	}
}
	...
	if (DO_SIGN(msg.destination) && msg.sender == getNodeId()) {
		if (skipSign(msg)) {
			...
			if (!_sendRoute(build(_msgSign, msg.destination, msg.sensor, C_INTERNAL,
				...
				// We have to wait_process for the nonce to arrive before we can sign our original message
...
//static uint8_t sha256_hash[32]; chinh sửa
//Sha256Class _soft_sha256;

void signerSha256Init(void)
{
//	memset(sha256_hash, 0, 32); // chỉnh sửa
//	_soft_sha256.init();
}
...
void signerSha256Update(const uint8_t* data, size_t sz)
{
	for (size_t i = 0; i < sz; i++) {
		//_soft_sha256.write(data[i]); // chỉnh sửa
	}
}

uint8_t* signerSha256Final(void)
{
//	memcpy(sha256_hash, _soft_sha256.result(), 32); // chỉnh sửa
//	return sha256_hash;
}
