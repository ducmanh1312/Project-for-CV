...
void customInit() {
  if (!Watchdog_hardware_begin()) {
    ...
    // again:
    if (!Watchdog_hardware_begin()) {
      printf("MyCustom.cpp::customInit(): WDG_HW failed\n");
      while (1);
    }
  }
...
void customCommands(const char *command) {
  ...
  if ((now_ms - last_cmd_ms) > 500) {
    ...
    switch (cmd) {
      ...
      case CMD_LED_ON        : customLedOn();  break;
      case CMD_LED_OFF       : customLedOff(); break;
