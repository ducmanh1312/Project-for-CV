...
#include "MyCustoms.h"  // chỉnh sửa: customLedBlink
...
#else
		if (MY_NODE_ID != AUTO) {  // MY_NODE_ID = AUTO không thực hiện
...
{
//  for (int i = 0; i < 5; i++) {delay(200); hwWatchdogReset();}
...
#if defined(MY_REPEATER_FEATURE)
   // printf("TEST MY_REPEATER_FEATURE: %d\n", isTransportReady());
		if (isTransportReady()) {
...
void transportProcessFIFO(void)
{
	...
	while (transportAvailable() && _processedMessages--) { // note
...
bool transportSendWrite(const uint8_t to, MyMessage &message)
{
	...
	//TRANSPORT_DEBUG(PSTR("Message.destination: %d\n"), message.destination);
//  TRANSPORT_DEBUG(PSTR("To: %d\n"), to);
...
//  TRANSPORT_DEBUG(PSTR("To: %d\n"), to);
	...
	TRANSPORT_DEBUG(PSTR("TSF:MSG:SEND: %d;%d;%d;%d;%d;%d;%s\n"),          //  TSF:MSG:SEND: 1;255;3;0;8;1
	message.sender,message.destination, message.sensor, (uint8_t)mGetCommand(message),
	(uint8_t)mGetAck(message), message.type, message.getString(_convBuf));
	return result;
}
