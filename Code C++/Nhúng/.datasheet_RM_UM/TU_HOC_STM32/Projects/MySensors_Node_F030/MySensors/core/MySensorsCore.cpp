...
void _registerNode(void)
{
	...
	} while (!wait_process(2000, C_INTERNAL, I_REGISTRATION_RESPONSE) && counter--);
#else
...
void presentNode(void)
{
	...
	signerPresentation(_msgTmp, GATEWAY_ADDRESS); // MySigning.h
	...
	(void)wait_process(2000, C_INTERNAL, I_CONFIG);
...
bool wait_process(const uint32_t waitingMS, const uint8_t cmd, const uint8_t msgType)
{
              ...
              const uint8_t mode1, const uint8_t interrupt2, const uint8_t mode2)
{
	...
	if (smartSleep) {
		...
		(void)sendHeartbeat();
		wait_process(MY_SMART_SLEEP_WAIT_DURATION_MS);		// listen for incoming messages
	}
