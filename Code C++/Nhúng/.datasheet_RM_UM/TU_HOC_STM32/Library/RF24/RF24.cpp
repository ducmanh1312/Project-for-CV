...
void RF24_LoadConfig() {
  ...
  #else
    // default config:
    my_rf24_config    = MY_RF24_CONFIGURATION;
    my_rf24_channel   = MY_RF24_CHANNEL;
    my_rf24_addrWidth = MY_RF24_ADDR_WIDTH;
    my_rf24_retries   = MY_RF24_RETRIES;
    my_rf24_rf_setup  = MY_RF24_RF_SETUP;

    uint8_t MY_RF24_BASE_ADDR[MY_RF24_ADDR_WIDTH] = {MY_RF24_BASE_RADIO_ID};
    for (int i = 0; i < my_rf24_addrWidth; i++)
    my_rf24_base_addr[i] = MY_RF24_BASE_ADDR[i];
  #endif
