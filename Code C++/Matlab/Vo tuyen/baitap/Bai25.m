function bai25
    ...
    r = 1;
    y0 = 0;
    h = 0.25; % Kích th??c b??c ?i cho ph??ng pháp Runge-Kutta b?c 3
    t_end = 2;
    ...
    % Th?c hi?n ph??ng pháp Runge-Kutta b?c 3
    for i = 1:(length(t_values)-1)
        k1 = dydt(y_values(i), t_values(i));
        k2 = dydt(y_values(i) + k1 * h / 2, t_values(i) + h / 2);
        k3 = dydt(y_values(i) - k1 * h + 2 * k2 * h, t_values(i) + h);
        
        y_values(i + 1) = y_values(i) + h * (k1 + 4*k2 + k3) / 6;
    end
